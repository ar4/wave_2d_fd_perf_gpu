/* One kernel, no shared memory, manual laplacian, 1D malloc */
#include <stdio.h>
                ...
                const int nt, const int ns, const int it)
{
        ...
        for (s = 0; s < ns; s++)
        {
                if ((z == sources_z[b * ns + s]) &&
                                (x == sources_x[b * ns + s]))
                {
                        wfp[ib] += source_amplitude[b * ns * nt + s * nt + it]
                                * model[i];
                        break; // assumes only one source per cell
                }
        }
